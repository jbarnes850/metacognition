"""
Full-pool behavioral sweep: d-prime across all 969 matched items.

Extends sweep_critique.py to use the entire DS Critique Bank item pool
instead of a 200-item subsample. Adds per-dataset d-prime breakdown
for the domain-specificity analysis.
"""
...
RESULTS_DIR = Path("results/sweep_v3_fullpool")
...
def generate_with_logprobs(model, tokenizer, messages, max_tokens=MAX_TOKENS):
    ...
    # Suppress thinking for all Qwen3.5 models (4B/9B templates leave <think> open)
    if "<think>\n" in prompt and "</think>" not in prompt:
        prompt = prompt + "</think>\n\n"
...
def main():
    print("=" * 70)
    print("METACOGNITION BENCHMARK v3: Full Pool (969 items)")
    ...
    # Use ALL items (no subsampling)
    items = all_items
    print(f"\nUsing all {len(items)} items.")

    models_to_test = {}
    for name, path in [("0.8B", "models/Qwen3.5-0.8B"), ("2B", "models/Qwen3.5-2B"),
                        ("4B", "models/Qwen3.5-4B"), ("9B", "models/Qwen3.5-9B")]:
        if Path(path).exists() and any(Path(path).glob("*.safetensors")):
            trial_path = RESULTS_DIR / f"trials_{name}.json"
            if trial_path.exists():
                print(f"Model {name}: SKIPPING (results exist at {trial_path})")
                continue
    ...
    # Load prior results from completed models
    for name in ["0.8B", "2B", "4B", "9B"]:
        summary_files = sorted(RESULTS_DIR.glob("sweep_v3_*.json"))
        for sf in summary_files:
            with open(sf) as f:
                prior = json.load(f)
            for r in prior:
                if r['model'] == name and name not in models_to_test:
                    all_results.append(r)
                    print(f"Loaded prior results for {name}: d'={r['d_prime']:.3f}")
    ...
    if len(all_results) > 1:
        print(f"\n{'='*70}")
        print("COMPARATIVE SUMMARY (Full Pool)")
        print(f"{'='*70}")
        header = f"{'Model':<8} {'Acc%':<7} {'N_sig':<7} {'N_noi':<7} {'d-prime':<22} {'HR':<7} {'FAR':<7} {'c':<7} {'Sci d':<8} {'Com d':<8}"
        print(f"\n{header}")
        print("-" * len(header))
        for r in all_results:
            ci = f"[{r['d_prime_ci95'][0]:.2f}, {r['d_prime_ci95'][1]:.2f}]"
            print(f"{r['model']:<8} {r['accuracy']*100:<7.1f} {r['n_signal_trials']:<7} {r['n_noise_trials']:<7} {r['d_prime']:<8.3f} {ci:<14} {r['hit_rate']:<7.3f} {r['false_alarm_rate']:<7.3f} {r['criterion_c']:<7.3f} {r['science_d_prime']:<8.3f} {r['commonsense_d_prime']:<8.3f}")

    # Save summary
    outpath = RESULTS_DIR / f"sweep_v3_{time.strftime('%Y%m%d_%H%M%S')}.json"
    ...
    if len(all_results) >= 2:
        ...
        print(f"\nMETRIC d_prime_spread={max(d_vals) - min(d_vals):.4f}")
        print(f"METRIC min_signal_n={min(r['n_signal_trials'] for r in all_results)}")
        for r in all_results:
